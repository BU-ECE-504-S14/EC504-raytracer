...
public class RayTracer {
	...
	private SceneObject getColor(Ray ray, int currentLevel, Vector3d viewerPosition,
		...
		/*if recursed to far do not light (ie color black) */
		if (currentLevel > MAX_LEVELS) {
			color.set(new double[] { 0, 0, 0 });
			return null;
		}
		
		/*if current ray does not intersect an object do not light (ie color black)*/
		...
		for (PointLight light : scene.getLights()) {
			...
			/* Use radiosity of light to poisition of intersection to compute shading factor */
			double shade = 0;
			for (int i = 0; i < shadow; i++) {
				lightPosition = new Vector3d(light.getPosition());
				
				/*randomize light position according to radiosity (used for diffuse lighting)*/
				if (shadow > 1) {
			...
			/*if light is too dim (requires bouncing off too many objects) do not light point */
		...
		/* The final result remains in ambient intensity */
	...
	/* recursively compute reflected color used in cojunction with getColor*/
	 ...
	 * In the pixel, constuct a grid with the antialiasing parameter and make the ray pass
