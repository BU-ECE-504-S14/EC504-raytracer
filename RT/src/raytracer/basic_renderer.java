...
public class basic_renderer
{
	...
	{
		new RenderViewer(renderSampleScene());
	}
	...
	public static BufferedImage renderSampleScene()
	{
		...
		Point3d position = new Point3d(40, -20, -8);
		...
		try
		{
			out = basic_renderer.renderScene(scene);
		}
		catch (IOException e)
		{
	...
	public static BufferedImage renderScene(Scene scene) throws IOException
	{
		...
		Simple_RayTracer rayTracer = new Simple_RayTracer(scene, imageSize, optionAntialiasing,
				optionShadow, null);

		/*
		 * more complex ray tracer specular lighting, anti aliasing, refraction,
		 * reflection, etc.
		 */
		 //RayTracer rayTracer = new RayTracer(scene, imageSize, optionAntialiasing,
		 //optionShadow);

		/* final raytracer: all features and fast */
		// RayTracer rayTracer = new RayTracer(new OctreeScene(scene), imageSize,
		// optionAntialiasing, optionShadow);
